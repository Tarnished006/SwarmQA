...
logger = logging.getLogger("aegis.agents.blue_team.checker")
...
async def execute_local_test(workspace_dir: str, test_filename: str, timeout_s: int = 20) -> Dict[str, Any]:
    ...
    Security: runs pytest in a sanitized environment that strips all host
    secrets before forking the subprocess (mirrors LocalSubprocessSandbox in
    active_test_agent.py).  Without this, any secret in the Aegis process
    environment (e.g. OPENAI_API_KEY, AWS credentials) would be visible to
    untrusted test code generated by the LLM.
    """
    # Vars known to carry secrets / credentials — never expose to test code.
    ...
    # Build a sanitized copy — start from host env then strip secrets.
    sanitized_env = {k: v for k, v in os.environ.items() if k not in _SENSITIVE_VARS}
    ...
    def _run():
        return subprocess.run(
            ...
            errors="replace",
            timeout=timeout_s,
            env=sanitized_env,       # ← sanitized: no host secrets
        )
...
Evaluate the effectiveness and safety of the applied patches. Return ONLY the structured VerificationReport JSON."""
    ...
    try:
        ...
        report: VerificationReport = await chain.ainvoke({
            "verified_exploits": sanitize_untrusted_input("\n".join(active_debugger), max_chars=8000),
            "remediation_plan": sanitize_untrusted_input("\n".join(remediation_plan), max_chars=8000),
            "test_execution_results": sanitize_untrusted_input("\n".join(test_results), max_chars=6000),
        })
